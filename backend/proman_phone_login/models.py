...
class PhoneToken(models.Model):
    ...
    def create_otp_for_number(cls, number):
        ...
        if otps.count() <= getattr(settings, 'PHONE_LOGIN_ATTEMPTS', 10):
            otp = cls.generate_otp(length=getattr(
                settings, 'PHONE_LOGIN_OTP_LENGTH', 6))
            phone_token = PhoneToken(phone_number=number, otp=otp)
            phone_token.save()
            # from_phone = '+8801786910645'
            message = 'Dear Customer %s .Use this code to verify yourself.Thanks' % (
                otp)
            data = {'token': token,
    ...
    def generate_otp(cls, length=6):
        hash_algorithm = getattr(
            settings, 'PHONE_LOGIN_OTP_HASH_ALGORITHM', 'sha256')
